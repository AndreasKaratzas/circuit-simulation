...
int read_vec_file(FILE *vec_file, PATTERN *vectors)
{
    ...
    while( !feof(vec_file) )
    {
        fgets(line, MAX_NUM_OF_CHAACTERS_IN_LINE, vec_file);
        sscanf(line, "%s", raw_vector);
        vectors[num_of_patterns].num_of_primary_inputs = strlen(raw_vector);
        vectors[num_of_patterns].primary_input_vec = malloc(vectors[num_of_patterns].num_of_primary_inputs * sizeof(int));
