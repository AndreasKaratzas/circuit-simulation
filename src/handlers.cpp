 ...
 */
void from_handler(NODE *graph, char *name, int address)
{
    int _branch_line, input_address;

    for (_branch_line = 0; _branch_line < address; _branch_line += 1)
    {
...
void gate_handler(FILE *isc_file, NODE *graph, int fanin, int address, int *node_dictionary)
{
    ...
    for (_branch_line = 0; _branch_line < fanin; _branch_line += 1)
    {
        ...
        insert_element(&graph[address].fanin, node_dictionary[input_address]);
        insert_element(&graph[node_dictionary[input_address]].fanout, address);
    }
